...
static void position_window_bottom_right(GtkWidget *window) {
    ...
    gint x_pos = screen_width - WINDOW_WIDTH - 4;
    gint y_pos = screen_height - WINDOW_HEIGHT - 22;
    if (x_pos < 0) x_pos = 0;
    if (y_pos < 0) y_pos = 0;
...
GtkWidget* create_custom_titlebar(GtkWidget *window) {
    GtkWidget *header_bar = gtk_header_bar_new();
    gtk_header_bar_set_show_close_button(GTK_HEADER_BAR(header_bar), TRUE);
    gtk_header_bar_set_title(GTK_HEADER_BAR(header_bar), "Dirty/Writeback");
...
void show_dirty_writecache_window(GtkMenuItem *item, gpointer user_data) {
    ...
    if (!disk_data->window) {
        ...
        create_custom_titlebar(disk_data->window);
        ...
        gtk_container_add(GTK_CONTAINER(disk_data->window), margin_box);
        gtk_widget_set_margin_start(margin_box, 10);
        gtk_widget_set_margin_end(margin_box, 10);
        gtk_widget_set_margin_top(margin_box, 10);
        gtk_widget_set_margin_bottom(margin_box, 10);
        ...
        PangoAttribute *attr_size = pango_attr_size_new(16 * PANGO_SCALE); // Taille de police 16
...
int main(int argc, char *argv[]) {
    ...
    gtk_status_icon_set_tooltip_text(disk_data->status_icon, "Disk activity");
