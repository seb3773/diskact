...
static gboolean check_disk_activity(gpointer user_data) {
    DiskData *disk_data = (DiskData *)user_data;
...
static gboolean update_dirty_writecache(gpointer user_data) {
    DiskData *disk_data = (DiskData *)user_data;
...
static void position_window_bottom_right(GtkWidget *window) {
    ...
    gint x_pos = geometry.x + geometry.width - actual_width - 4;
    gint y_pos = geometry.y + geometry.height - actual_height - 22;
    if (x_pos < 0) x_pos = 0;
    if (y_pos < 0) y_pos = 0;
...
void show_dirty_writecache_window(GtkMenuItem *item, gpointer user_data) {
    ...
    if (!disk_data->window) {
        ...
        gtk_widget_set_halign(disk_data->dirty_label, GTK_ALIGN_CENTER);
        gtk_widget_set_valign(disk_data->dirty_label, GTK_ALIGN_CENTER);
        gtk_widget_set_halign(disk_data->writecache_label, GTK_ALIGN_CENTER);
        gtk_widget_set_valign(disk_data->writecache_label, GTK_ALIGN_CENTER);
